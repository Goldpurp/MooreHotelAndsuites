...
const Amenities: React.FC = () => {
  ...
  useEffect(() => {
    const loadImages = async () => {
      try {
        const [pool, games, lounge, spa] = await Promise.all([
          generateAppImage(APP_IMAGE_PROMPTS.amenity_pool),
          generateAppImage(APP_IMAGE_PROMPTS.amenity_games),
          generateAppImage(APP_IMAGE_PROMPTS.dining_sanctuary),
          generateAppImage(APP_IMAGE_PROMPTS.amenity_spa)
        ]);
        setAmenityImages({
          "The Infinity Oasis": pool || '',
          "The Apex Games Room": games || '',
          "The Meridian Lounge": lounge || '',
          "Wellness & Sanctuary": spa || ''
        });
      } catch (err) {
        console.error("Failed to load amenity imagery");
      } finally {
        setLoading(false);
      }
    };
  ...
  const sections = [
    {
      ...
      img: amenityImages["The Infinity Oasis"] || "https://images.unsplash.com/photo-1576013551627-0cc20b96c2a7?auto=format&fit=crop&q=80&w=1200",
      features: ["Salt Water Sanitized", "Private Cabanas", "Poolside Service", "City Vistas"]
    },
    {
      ...
      img: amenityImages["The Apex Games Room"] || "https://images.unsplash.com/photo-1517436073-3b1b11111111?auto=format&fit=crop&q=80&w=1200",
      features: ["Bespoke Pool Tables", "Private Poker Vault", "Board Game Library", "Cigar Selection"]
    },
    {
      ...
      img: amenityImages["The Meridian Lounge"] || "https://images.unsplash.com/photo-1551024709-8f23befc6f87?auto=format&fit=crop&q=80&w=1200",
      features: ["Artisanal Spirits", "Curated Vinyls", "Intimate Pods", "Midnight Service"]
    },
    {
      ...
      img: amenityImages["Wellness & Sanctuary"] || "https://images.unsplash.com/photo-1540555700478-4be289fbecef?auto=format&fit=crop&q=80&w=1200",
      features: ["Thermal Suites", "Hydrotherapy", "Meditation Garden", "Bespoke Rituals"]
    }
  ];
    ...
    <div className="bg-background-dark pt-24 min-h-screen">
      ...
      <section className="relative h-[55vh] md:h-[60vh] flex items-center justify-center overflow-hidden">
        <div className="absolute inset-0">
          <img 
            src={amenityImages["Wellness & Sanctuary"] || "https://images.unsplash.com/photo-1566073771259-6a8506099945?auto=format&fit=crop&q=80&w=1920"} 
        ...
        <div className="relative z-10 text-center px-6">
          <p className="text-[8px] md:text-[10px] uppercase tracking-[0.6em] text-primary font-black mb-6 md:mb-8 animate-pulse">Unrivaled Hospitality</p>
          <h1 className="serif-font text-5xl md:text-[10rem] text-white italic drop-shadow-2xl leading-none">Refined Living</h1>
      ...
      <section className="py-24 md:py-40">
        <div className="max-w-[1800px] mx-auto px-6 md:px-10 space-y-32 md:space-y-52">
          {sections.map((section, idx) => (
            <div key={section.title} className={`flex flex-col ${idx % 2 === 1 ? 'lg:flex-row-reverse' : 'lg:flex-row'} items-center gap-10 md:gap-24 lg:gap-32`}>
              {/* Image Side */}
              ...
              <div className="flex-1 space-y-8 md:space-y-10">
                <div className="space-y-4 md:space-y-6">
                  <p className="text-primary text-[10px] md:text-[11px] font-black uppercase tracking-[0.4em]">{section.tag}</p>
                  <h2 className="serif-font text-5xl md:text-7xl lg:text-[8rem] text-white leading-tight italic">{section.title}</h2>
                </div>
                <p className="text-gray-400 text-lg md:text-2xl font-light leading-relaxed max-w-2xl">{section.desc}</p>
                
                <div className="grid grid-cols-2 gap-4 md:gap-6 pt-6 md:pt-10">
                  {section.features.map(feature => (
                    <div key={feature} className="flex items-center gap-3 text-[10px] md:text-[12px] font-black uppercase tracking-[0.2em] text-white/40 group-hover:text-white transition-colors">
      ...
      <section className="py-24 md:py-40 px-6">
        <div className="max-w-[1800px] mx-auto text-center space-y-12 md:space-y-16 py-16 md:py-20 bg-surface-dark border border-white/5 rounded-sm shadow-2xl relative overflow-hidden">
           ...
           <div className="inline-flex items-center justify-center w-20 h-20 md:w-28 md:h-28 rounded-full bg-primary/5 border border-primary/20 text-primary mb-4 md:mb-6 animate-luxury-logo">
             <span className="material-symbols-outlined text-4xl md:text-5xl">support_agent</span>
           </div>
           <h2 className="serif-font text-4xl md:text-7xl text-white italic leading-tight">At Your Service, <br className="hidden md:block" />Every Moment</h2>
